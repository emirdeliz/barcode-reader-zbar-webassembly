...
EMCC_FLAGS = -Os -Wall -Werror --no-entry -s ALLOW_MEMORY_GROWTH=1 \
	...
	-s EXPORT_NAME="czbar"
	-s MINIMAL_RUNTIME=1
CZBAR_WASM_DEPS = webassembly/barcode-reader.wasm
CZBAR_BARCODE_READER_ENTRY = node_modules/barcode-reader-zbar-c/src/c_barcode_scanner.c

dist/all: $(ZBAR_DEPS) $(CZBAR_WASM_DEPS)

$(CZBAR_WASM_DEPS):
	$(EMCC) $(EMCC_FLAGS) -o webassembly/barcode-reader.js $(CZBAR_BARCODE_READER_ENTRY) $(ZBAR_INC) $(ZBAR_OBJS)
	rm -rf webassembly/barcode-reader.js
