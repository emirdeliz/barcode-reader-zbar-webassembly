 ...
 */
int c_get_check_digit_mod_10(char barcode_segment[])
{
	int j = 0;
	int mult = 2;
	int sum = 0;
	...
	int *barcode_segment_as_int_array = c_char_array_to_int_array(barcode_segment);
	...
	for (int i = barcode_segment_as_length - 1; i >= 0; i--)
	{
		...
		int calc_mult_as_num_digits = c_count_num_digits_int(calc_mult);
		char *calc_mult_as_string = c_int_to_string_11(calc_mult);
 ...
 * @returns int - check digit.
 */
int c_get_check_digit_mod_11(char barcode_segment[])
{
	int sum = 0;
	int weight = 2;
	int base = 9;
 ...
 */
EXPORT int c_calc_check_digit(char segment[], int mod)
{
	if (mod == 10)
	{
		return c_get_check_digit_mod_10(segment);
	}
	else if (mod == 11)
	{
		return c_get_check_digit_mod_11(segment);
	}
	return 0;
}
 ...
 */
EXPORT int c_get_mod(int ref_number)
{
	switch (ref_number)
	{
	case 6:
		...
		int barcode_length)
{
	...
	int mod = c_get_mod(barcode_char_array[2] - '0');
	...
	for (int i = 0; i <= check_digit_length - 1; i += 1)
	{
		...
		char segment[11] = "\0";
		c_substring(barcode_char_array, start_index, end_index, segment);

		int check_digit = c_calc_check_digit(segment, mod);
 ...
 */
EXPORT int c_check_if_barcode_is_from_insurance(char *barcode_char_array)
{
...
void c_apply_maybe_check_digit_to_barcode(char barcode[], char target[], int barcode_length)
{
	...
	{
		c_apply_check_digit(barcode, target, barcode_length);
	}
	else
	{
