 ...
 * @param zbar_image_t *image - The image pointer. This param is created across the c_zbar_image_create. on the frontend.
 ...
 */
EXPORT int c_zbar_image_scanner_scan_and_maybe_apply_check_digit(zbar_image_t *image, char *result, int ignore_pix)
{
	...
	c_apply_maybe_check_digit_to_barcode(barcode_no_check_digits, result, strlen(barcode_no_check_digits));
	return 0;
}